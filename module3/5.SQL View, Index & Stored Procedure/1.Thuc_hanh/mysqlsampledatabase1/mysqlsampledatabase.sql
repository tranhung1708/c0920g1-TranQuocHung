...
-- thực hành 2
CREATE PROCEDURE findAllCustomers() SELECT * FROM customers; 
DROP PROCEDURE IF EXISTS `findAllCustomers`;
CREATE PROCEDURE findAllCustomers()SELECT *  FROM customers where customerNumber = 175;

-- thực hành 3

CREATE PROCEDURE getCusById (IN cusNum INT(11)) SELECT * FROM customers WHERE customerNumber = cusNum;
call getCusById(175);

CREATE PROCEDURE GetCustomersCountByCity(IN  in_city VARCHAR(50),OUT total INT) SELECT COUNT(customerNumber)INTO total FROM customers
WHERE city = in_city; 
CALL GetCustomersCountByCity('Lyon',@total);

SELECT @total;

CREATE PROCEDURE SetCounter(INOUT counter INT,IN inc int)  SET counter = counter + inc;
SET @counter = 1;
